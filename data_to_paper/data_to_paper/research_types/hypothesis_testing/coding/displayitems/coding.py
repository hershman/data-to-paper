...
class CreateLatexDisplayitemsCodeAndOutput(CodeAndOutput):
    def get_code_header_for_file(self, filename: str) -> Optional[str]:
        ...
        if filename.startswith('tbl_') and filename.endswith('.tex'):
            return f'## Table {filename[3:-4]}:'
        if filename.startswith('fig_') and filename.endswith('.tex'):
            return f'## Figure {filename[3:-4]}:'
        return None
...
@dataclass
class CreateDisplayitemsCodeProductsGPT(BaseScientificCodeProductsGPT, CheckLatexCompilation):
    ...
    background_product_fields: Tuple[str, ...] = \
        ('data_file_descriptions', 'research_goal', 'codes:data_preprocessing', 'codes:data_analysis',
         'created_files_content:data_analysis:*.pkl')
    ...
    provided_code: str = dedent_triple_quote_str('''
        def df_to_latex(df, filename: str, caption: str,
            ...
            Parameters:
            - df, filename, caption: as in `df.to_latex`.
            - note (optional): Additional note below the table.
            - glossary (optional): Dictionary mapping abbreviations to full names.
            - **kwargs: Additional arguments for `df.to_latex`.
            """

        def df_to_figure(df, filename: str, caption: str,
            ...
            `x_ci` / `y_ci` (optional, str): a column name where each row contains
                a 2-element tuple (n x 2 matrix when expanded).
