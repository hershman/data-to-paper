...
from .utils import get_pickle_dump_attr_replacer
...
class DataFramePickleContentOutputFileRequirement(PickleContentOutputFileRequirement):
    ...
    def _to_str(self, content: ListInfoDataFrame, pvalue_on_str: OnStr = OnStr.SMALLER_THAN, with_hyperlinks: bool = False) -> str:
        with OnStrPValue(pvalue_on_str):
            return describe_df(content)
    ...
    def _get_content_and_header_for_app_html(self, content: Any, filename: str = None, num_file: int = 0, level: int = 3):
        ...
        html = func(*args, **kwargs, is_html=True, save_fig=False)
    ...
    def _get_content_and_header_for_code_review(self, content: Any, filename: str = None, num_file: int = 0, level: int = 3):
        ...
        return wrap_as_block(content, 'output'), '#' * level + f' {filename}'
    ...
    def _get_content_and_header_for_product(self, content: Any, filename: str = None, num_file: int = 0, level: int = 3, with_hypertarget: bool = False):
        func, args, kwargs = self._get_func_args_kwargs(content)
        code = format_text_with_code_blocks(func(*args, **kwargs, save_fig=False))
        return code, '#' * level + f' {filename}'
...
class DictPickleContentOutputFileRequirement(PickleContentOutputFileRequirement):
    def _to_str(self, content: Any, pvalue_on_str: OnStr = OnStr.SMALLER_THAN, with_hyperlinks: bool = False) -> str:
        ...
        return super()._to_str(content, pvalue_on_str, with_hyperlinks)
...
class DataAnalysisCodeProductsGPT(BaseScientificCodeProductsGPT):
    ...
    output_file_requirements: OutputFileRequirements = OutputFileRequirements(
        [DataFramePickleContentOutputFileRequirement('df_*.pkl', 1),
         DataOutputFileRequirement('df_*.png', 0),
